...
BUNDLE_URL="${PLUXX_CURSOR_BUNDLE_URL:-https://github.com/${REPO}/releases/download/v0.2.2/pluxx-cursor-latest.tar.gz}"
INSTALL_DIR="${PLUXX_CURSOR_INSTALL_DIR:-$HOME/.cursor/plugins/local/$PLUGIN_NAME}"
BUNDLE_PATH="${PLUXX_CURSOR_BUNDLE_PATH:-}"

PLUXX_TX_PLATFORM="cursor"
PLUXX_TX_STAGE=""
PLUXX_TX_BACKUP=""
PLUXX_TX_SWAPPED=0
...
BUNDLE_ARCHIVE="$TMP_DIR/pluxx-cursor.tar.gz"
...
verify_release_asset "$BUNDLE_ARCHIVE" "pluxx-cursor-latest.tar.gz"

PLUXX_RELEASE_MANIFEST="$RELEASE_MANIFEST" PLUXX_EXPECTED_PLUGIN="$PLUGIN_NAME" PLUXX_EXPECTED_PLATFORM="cursor" PLUXX_EXPECTED_VERSION="0.2.2" node <<'NODE'
const fs = require('fs')
...
if (!archive || archive.latestAsset !== 'pluxx-cursor-latest.tar.gz') {
