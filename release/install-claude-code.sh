...
MARKETPLACE_NAME="${PLUXX_CLAUDE_MARKETPLACE_NAME:-pluxx-releases}"
BUNDLE_URL="${PLUXX_CLAUDE_BUNDLE_URL:-https://github.com/${REPO}/releases/download/v0.2.2/pluxx-claude-code-latest.tar.gz}"
...
AUTHOR_NAME="${PLUXX_PLUGIN_AUTHOR:-Orchid Automation}"
HOMEPAGE_URL="${PLUXX_PLUGIN_HOMEPAGE:-https://pluxx.dev}"
DESCRIPTION_FALLBACK="${PLUXX_PLUGIN_DESCRIPTION:-One plugin workflow source. Native bundles for Claude, Cursor, Codex, and OpenCode.}"

PLUXX_TX_PLATFORM="claude-code"
PLUXX_TX_STAGE=""
PLUXX_TX_BACKUP=""
PLUXX_TX_SWAPPED=0
...
BUNDLE_ARCHIVE="$TMP_DIR/pluxx-claude-code.tar.gz"
...
verify_release_asset "$BUNDLE_ARCHIVE" "pluxx-claude-code-latest.tar.gz"

PLUXX_RELEASE_MANIFEST="$RELEASE_MANIFEST" PLUXX_EXPECTED_PLUGIN="$PLUGIN_NAME" PLUXX_EXPECTED_PLATFORM="claude-code" PLUXX_EXPECTED_VERSION="0.2.2" node <<'NODE'
const fs = require('fs')
...
if (!archive || archive.latestAsset !== 'pluxx-claude-code-latest.tar.gz') {
