...
- whether the workflow needs the Pluxx 0.1.9 companion surface for behavioral proof, installed MCP discovery, and current hook fixes
...
3. If `pluxx` is present but stale, prefer:
   ...
   - require Pluxx CLI `>=0.1.9` for the current plugin companion workflows
...
- If behavioral proof, local stdio runtime bundling, or Claude hook env rehydration is in scope, prefer upgrading to Pluxx CLI `>=0.1.9` before continuing.
