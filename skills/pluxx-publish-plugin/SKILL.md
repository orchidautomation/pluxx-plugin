...
4. Verify the project with `pluxx validate`, `pluxx doctor`, `pluxx lint`, `skills-ref validate` for every source skill, and `pluxx test`.
...
6. Preview first:
   ...
   - add `--github-release`, `--npm`, and `--version <x.y.z>` to match the intended channels; `--tag <name>` selects only the npm dist-tag
